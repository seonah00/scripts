...
export const concepts=[{title:'궁금증을 여는 발견형',tag:'DISCOVERY',description:'익숙한 고민에서 시작해 제품·공간의 특징을 하나씩 소개해요.',hook:'처음 보는 사람도 이해할 수 있게'},{title:'저장하고 싶은 체크리스트',tag:'SAVE-WORTHY',description:'구매·방문 전 확인할 내용을 정리해 정보의 가치를 높여요.',hook:'필요할 때 다시 꺼내 보는 콘텐츠'},{title:'디테일에 집중하는 관찰형',tag:'DETAIL-LED',description:'제형·패키지·공간 등 직접 보여줄 수 있는 장면을 중심으로 구성해요.',hook:'과장보다 구체적인 장면으로'}];
export type Finding={id:string;term:string;start:number;end:number;category:string;reason:string;suggestion:string;level:'high'|'context'};
const rules:{regex:RegExp;category:string;reason:string;suggestion:string;level:'high'|'context'}[]=[
 {regex:/100\s*[%％]\s*(有效|효과|보장)?|百分之百有效|무조건\s*효과|guaranteed\s+results/gi,category:'효과 단정',reason:'모든 사람에게 같은 효과를 보장하는 표현입니다. 근거와 적용 범위를 확인해 주세요.',suggestion:'效果因人而异',level:'high'},
 {regex:/第一|最好的|全网最低|최고의|세계\s*1위|the best/gi,category:'최상급·순위',reason:'객관적 순위나 비교 근거가 필요한 표현입니다. 순서·일반 문맥인지도 확인해 주세요.',suggestion:'值得了解的',level:'context'},
 {regex:/治愈|根治|영구\s*치료|완치|cures?\b/gi,category:'치료 효능',reason:'치료를 단정하는 문구는 제품의 허용된 효능·근거를 별도로 확인해야 합니다.',suggestion:'日常护理',level:'high'},
 {regex:/微信|加微|私信购买|카톡으로\s*구매/gi,category:'외부 구매 유도',reason:'외부 연락·거래 유도에 해당하는지 게시 맥락과 플랫폼 정책을 확인해 주세요.',suggestion:'请查看产品信息',level:'context'},
 {regex:/내돈내산|亲测|自费购入|用了7天|7일\s*(사용|써)|I tried|I used/gi,category:'실사용 주장',reason:'본인의 실제 구매·사용 경험으로 확인할 수 있는 내용인지 검토해 주세요.',suggestion:'구매 전 확인한 정보',level:'context'},
